...
const ProductOverlayCard: React.FC<ProductProps> = ({
	...
	const name = locale==='en' ? product.nameEng: product.nameAr
	const details = locale==='en' ? product.detailsEng: product.detailsAr
	...
	return (
		...
		>
			...
			>
				<Image
					src={
						product?.image ? (process.env.NEXT_PUBLIC_GET_ATTACHMENT+product?.image?.id) :
						(product?.images?.length>0 && process.env.NEXT_PUBLIC_GET_ATTACHMENT+product?.images[0]?.id) ||
