...
const Card: React.FC<Props> = ({
	...
	const { nameEng, image, nameAr } = item ?? {};
	const imageSize: any =
		(size === "small" && 180) || (size === "medium" && 198)	|| (size === "large" && 240);
	...
	return (
		...
		>
			...
			>
				<div className="flex">
					<Image
						src={'image' in item ? process.env.NEXT_PUBLIC_GET_ATTACHMENT+item?.image?.id : item?.images?.length>0?process.env.NEXT_PUBLIC_GET_ATTACHMENT+item?.images[0]?.id: placeholderImage}
						alt={locale==='en'?nameEng:nameAr || t("text-card-thumbnail")}
						width={imageSize}
						height={imageSize}
						quality={100}
