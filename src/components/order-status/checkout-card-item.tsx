...
export const OrderItem: React.FC<{ item: Item }> = ({ item }) => {
	...
	return (
		<div className="flex py-4 items-center lg:px-3 border-b border-gray-300">
			<div className="flex border rounded-md border-gray-300 w-16 h-16 flex-shrink-0">
				<img
					src={process.env.NEXT_PUBLIC_GET_ATTACHMENT+item.product?.images[0] ?? "/assets/placeholder/order-product.svg"}
					width="64"
					height="64"
			...
			<h6 className="text-sm ps-3 font-regular text-heading">
				{generateCartItemName(locale==='en'?item.product?.nameEng:item.product.nameAr, {})}
			</h6>
