...
type Action =
  ...
  | { type: "REMOVE_ITEM_OR_QUANTITY"; id: Item["id"]; attributes:any; quantity?: number }
  | { type: "ADD_ITEM"; id: Item["id"]; item: Item }
  | { type: "UPDATE_ITEM"; id: Item["id"]; item: UpdateItemInput }
  | { type: "REMOVE_ITEM"; id: Item["id"]; attributes:any}
  | { type: "RESET_CART" };
...
export function cartReducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "REMOVE_ITEM": {
      const items = removeItem(state.items, action.id, action.attributes);
      return generateFinalState(state, items);
    }
    case "UPDATE_ITEM": {
